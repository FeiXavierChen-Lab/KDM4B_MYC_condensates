...
usage() {
    ...
    echo "Usage: $0 <all_path> <runInfo>  <expRef> <spkRef> <email> <spikeIn>  <spkStrategy> <libType> <rmDup> <controlIgG> <callPeak> <peakType>"

    echo "  all_path:  your project path "
    ...
    echo "  email: your email to receive notice "
...
if [[ -d "$all_path" && $(ls -A "$all_path") ]]; then
        echo "Using $all_path as input data."
else
        echo "Error: Invalid or empty all_path directory!"
	    usage
fi

#if [[ ! -s "$runInfo" ]];then
#    echo "Error: $runInfo is not a valid file!"
#    usage
#fi
...
workDir=${all_path}/Alignment/${runInfo}
input_path=${all_path}/Raw_data/${runInfo}
sampleInfo=${input_path}/sampleinfo.txt
...
/gpfs/chenfeilab/Coconut/Software/dos2unix-7.5.2/dos2unix $sampleInfo
...
# # fastqc 
fastqc='${file_path}/fastqc'
# # multiqc 
multiqc='${file_path}/multiqc'
# # trim_galore version 0.6.10 with cutadapt version 4.9
trim_galore='${file_path}/trim_galore'
# # cutadapt 
cutadapt='${file_path}/cutadapt'
# # bowtie2 
bowtie2='${file_path}/bowtie2'
# # samtools 
samtools='${file_path}/samtools'
# # picard  
picard='${file_path}/picard'
# # deepTools 
bamCoverage='${file_path}/bamCoverage'
alignmentSieve='${file_path}/alignmentSieve'
# # bedtools 
bedtools='${file_path}/bedtools'
# # bedops 
bedops='${file_path}/bedops'
# # MACS 
macs3='${file_path}/macs3'
...
seacr='${file_path}/SEACR_1.4.sh'

# # R script
Rscript='${file_path}/Rscript'
plot_insert_size_distribution='${file_path}/plot_insert_size_distribution.R'

# Bowtie2 index
hg38Index='${file_path}/hg38'
hg19Index='${file_path}/hg19'
mm10Index='${file_path}/mm10'
dm6Index='${file_path}/dm6'
k12Index='${file_path}/ASM584v2'
...
hg38Blacklist='${file_path}/ENCFF356LFX.bed'
hg19Blacklist='${file_path}/hg19-blacklist.v2.bed'
mm10Blacklist='${file_path}/mm10-blacklist.v2.bed'
# CUT&RUN blacklist (only for CUT&RUN)
hg38BlacklistCR='${file_path}/hg38_Nordin_CUTRUN_blacklist.bed'
mm10BlacklistCR='${file_path}/mm10_Nordin_CUTRUN_blacklist.bed'
...
if [[ ! -d $rawDataDir ]]; then
    ...
    cat $sampleInfo | while read file; do
        ...
        old=${arr[0]}
        new=${arr[1]}
        full=`ls ${input_path}/*/* | grep $old`
        suffix=$(echo -e "$full" | rev | sed -e '1{h;d;}' -e 'G;s,\(.*\).*\n\1.*,\1,;h;$!d' | rev)
        r1=`ls ${input_path}/*/*1${suffix} | grep $old`
        r2=`ls ${input_path}/*/*2${suffix} | grep $old`
...
if [[ ! -s $advSummaryDir ]]; then
    ...
    cat $summaryDir | sed '1d' | while read line; do
        ...
        # peak counts
        if [[ $peakType == 'narrow' ]]; then
            peakBed=${macs3Dir}/${sampleName}_filter_peaks.narrowPeak
            nCol=11
        else
            peakBed=${macs3Dir}/${sampleName}_filter_peaks.broadPeak
            nCol=10
        fi
        if [[ -s $peakBed ]]; then
            peakCounts=`cat $peakBed | wc -l`
...
# Send email notification
if ls ${cpmScaledBwDir}/*bw >/dev/null 2>&1; then
    echo "Sending email notification..."
    echo -e "The cutrun or cuttag-seq pipeline for datainfo ${runInfo} has completed successfully.\n
            Experimental reference: ${expRef}
            Spike-in reference: ${spkRef}\n
            Please check the results in the following directory: ${cpmScaledBwDir}" | \
            python ${file_path}/sendmail.py ${email} "ChIP-seq Pipeline Complete"
else
    echo -e "NOTE NOTE NOTE:
            The cutrun or cuttag-seq pipeline for datainfo ${runInfo} has completed WRONGLY.\n
            Experimental reference: ${exp_ref}
            Spike-in reference: ${spkRef}\n
            PLEASE CHECK YOUR INPUTS AND RUN AGAIN."
fi
